...
class SpriteGame:
    ...
    def main(self):
        ...
        while running : # mian game looop
            ...
            self.window.blit(best_scoresurf, bestrect)
            self.window.blit(textSurf, self.textRectObj) # writing score 
            if self.direction == "horizontal":
                if hor >= 630:
                    hor = 0
                if hor <= -10:
                    hor = 640
                self.displ(im1, h = hor, v = ver) ; hor += b 
                self.displ(im11, h = hor, v = ver);hor += b; sleep(0.03)
                self.displ(im2, h = hor, v = ver) ;hor += b
                self.update()
                ...
                if ver <= -10:
                    ver = 470
                self.displ(im1,h = hor, v = ver) ; ver += b
                self.displ(im11,h = hor, v = ver);ver += b; sleep(0.028)
                self.displ(im2,h = hor, v = ver) ;ver += b 
                self.update()
            ...
            if (abs(hor - ene_pos_hor) <= 20) and (abs(ver - ene_pos_ver) <= 20):
                ...
                self.window.blit(textSurf, self.textRectObj)
                running = False
    ...
    def update(self):
        self.displ(im3, 0,0)
        self.displ(ye_1, ye_hor, ye_ver)
        self.displ(enemy_1, ene_pos_hor, ene_pos_ver)
        pygame.display.update()
